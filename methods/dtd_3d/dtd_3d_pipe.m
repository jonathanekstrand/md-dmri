function fn = dtd_3d_pipe(s, paths, opt)
% function fn = dtd_3d_pipe(s, paths, opt)
%
% s     - input structure
...
% Run the analysis
if (opt.do_data2fit)
    mdm_data2fit(@dtd_3d_4d_data2fit, s, paths.mfs_fn, opt);
end
if (opt.do_fit2param)
    mdm_fit2param(@dtd_3d_4d_fit2param, paths.mfs_fn, paths.dps_fn, opt);
end

% Save nifti parameter maps    
if (opt.do_param2nii)
    fn = mdm_param2nii(paths.dps_fn, paths.nii_path, opt.dtd_3d, opt);
...
% Save dtd_3d pdf   
if (opt.do_m2pdf)
    dtd_3d_m2pdf(paths.dps_fn, paths.nii_path, opt);
end
