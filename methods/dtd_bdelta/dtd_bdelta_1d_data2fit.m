function m = dtd_bdelta_1d_data2fit(signal, xps, opt, ind)
% function m = dtd_bdelta_1d_data2fit(signal, xps, opt, ind)
...
unit_to_SI = [max(signal) 1e-9 1];
    ...
    function s = my_1d_fit2data(t,varargin)
        ...
        % signal
        s = dtd_bdelta_1d_fit2data(m, xps);
        s = s(ind);
...
t = lsqcurvefit(@my_1d_fit2data, t_guess, [], signal(ind), t_lb, t_ub, opt.dtd_bdelta.lsq_opts);

m_oblate = t2m(t);

signal_fit = dtd_bdelta_1d_fit2data(m_oblate, xps);
...
t = lsqcurvefit(@my_1d_fit2data, t_guess, [], signal(ind), t_lb, t_ub,...
    opt.dtd_bdelta.lsq_opts);

m_prolate = t2m(t);

signal_fit = dtd_bdelta_1d_fit2data(m_prolate, xps);
...
if (opt.dtd_bdelta.do_plot)
    signal_fit = dtd_bdelta_1d_fit2data(m, xps);
