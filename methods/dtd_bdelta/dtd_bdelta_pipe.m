function fn = dtd_bdelta_pipe(s, paths, opt)
...
if (opt.dtd_bdelta.do_pa)
    s = mdm_s_powder_average(s, fileparts(s.nii_fn), opt);
end

% Run the analysis
if (opt.do_data2fit)
    mdm_data2fit(@dtd_bdelta_4d_data2fit, s, paths.mfs_fn, opt);
end
if (opt.do_fit2param)
    mdm_fit2param(@dtd_bdelta_4d_fit2param, paths.mfs_fn, paths.dps_fn, opt);
end
